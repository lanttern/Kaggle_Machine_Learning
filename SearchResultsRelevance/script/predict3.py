...
if __name__ == '__main__':
    ...
    # Fit TFIDF
    import scipy.sparse
    def vectorize(train,features, tfv_query=None):
        ...
        return scipy.sparse.hstack([tfv_query.transform(query_data), tfv_query.transform(title_data), features]), tfv_query
    
    features1 = extract_features_labels("../data/features.csv")    
    X, tfv_query = vectorize(train, features1)
    
    features_test1 = extract_features_labels("../data/features_test.csv")
    X_test, _ = vectorize(test, features_test1, tfv_query)
