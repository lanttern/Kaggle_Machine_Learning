...
if __name__ == "__main__":
    ...
    print "fit model..."   
    parameter = {"alpha": [1e4, 1e2, 1, 0.1, 0.05, 0.005, 0.0003, 0.00005]}      
    nb = naive_bayes.MultinomialNB()
    clf = grid_search.GridSearchCV(nb, parameter)
    clf.fit(features_train, labels_train)    
    pred = pred(clf, features_train, labels_train, features_test)
    a, b, c = validation (pred, labels_test)
    print a, b, c
    
    """
    print "Training model..."
    #create sklearn pipeline, fit all, and predit test data
    clf = naive_bayes.MultinomialNB(alpha = 0.0003)
